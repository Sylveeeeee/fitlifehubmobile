      ...
      <ScrollView className="px-4 pt-4" contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View className="mb-4">
          <Text className="text-white text-sm">
            Group: <Text className="text-gray-400">{food.category || 'Uncategorized'}</Text>
          </Text>
          <Text className="text-white text-sm">
            Data Source: <Text className="text-gray-400">{food.source || 'N/A'}</Text>
          </Text>
        ...
        <View className="bg-[#2a2c3d] rounded-xl p-4 mb-6">
          ...
          {[
            { label: 'Energy', value: food.calories, target: targets.calories, unit: 'kcal', color: '#fff' },
            ...macros.map((m) => ({
              label: m.label,
              value: m.value,
              target: m.target,
            ...
            <View key={item.label} className="mb-3">
              <Text className="text-white text-sm mb-1">
                ...
                {((item.value / (item.target || 1)) * 100).toFixed(0)}%)
              </Text>
              <View className="h-2 w-full bg-gray-700 rounded-full">
                <View
                  style={{
                    width: `${Math.min((item.value / (item.target || 1)) * 100, 100)}%`,
        ...
        <Pressable
          onPress={() => console.log('Add to Diary')}
          className="bg-white py-3 rounded-full items-center mb-10"
        >
