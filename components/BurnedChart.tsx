...
import { View, Text, Dimensions, ActivityIndicator, TouchableOpacity, Modal, ScrollView } from 'react-native';
...
export default function BurnedChart() {
  ...
  // เลือกว่าจะใช้ ScrollView หรือไม่
  const isScrollable = range === '1m' || range === '14d';

  const ChartComponent = (
    <VictoryChart
      domainPadding={{ x: 20, y: 5 }}
      height={200}
      width={range === '1m' ? screenWidth * 2.6 : range === '14d' ? screenWidth * 1.7 : screenWidth * 0.9} 
  ...
  return (
    <View className="bg-[#232433] rounded-2xl p-5 mb-5 shadow-lg w-[100%] justify-center items-center">
        ...
        <Modal transparent animationType="fade" visible={modalVisible} onRequestClose={() => setModalVisible(false)}>
          <View className="flex-1 justify-center items-center bg-black/50">
            <View className="bg-[#232433] p-4 rounded-xl w-[80%]">
              ...
              <Text className="text-white text-sm">BMR: {selectedData.baseEnergyNeed} kcal</Text>
              <Text className="text-white text-sm">Activity: {selectedData.activityCalories} kcal</Text>
              <Text className="text-white text-sm">Exercise: {selectedData.burned} kcal</Text>
              ...
              <TouchableOpacity className="mt-3 bg-[#ffb300] p-2 rounded" onPress={() => setModalVisible(false)}>
